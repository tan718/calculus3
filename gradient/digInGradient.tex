...
\begin{definition}
  ...
  \[
  D_\vec{u}(F) = \grad(F(x,y))\dotp \vec{\hat{u}}
  \]
...
\begin{example}
  ...
  \begin{explanation}
    Write with me
    \[
    \nabla F(x,y) = \vector{ -2x+2,-2y+2}.
    \]
    However, we see $\nabla F(1,1) = \vector{0,0}$.  Since there is no
...
Stand back. We're going to do some serious calculus now.
