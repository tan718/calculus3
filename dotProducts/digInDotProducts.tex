...
\begin{theorem}
  ...
  %%   \[
  %%   (c\vec{w} - \vec{v})\dotp \vec{w} = 0
  %%   \]
  %%   Thus
  %%   \begin{align*}
  %%     (c\vec{w} - \vec{v})\dotp \vec{w} &= 0\\
  %%     c(\vec{w} \dotp \vec{w}) - \vec{v} \dotp \vec{w} &=0\\
  %%     c = \frac{\vec{v} \dotp \vec{w}}{|\vec{w}|^2}
  %%   \end{align*}
  %% \end{explanation}
\end{theorem}
...
\begin{definition}
  ...
  in the direction of $\vec{w}$ of a vector $\vec{v}$ is denoted
  \[
  \scal_\vec{w}(\vec{v})=
  \begin{cases}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    \begin{align*}
      \proj_\vec{g}(\vec{r}) &= \left(\frac{\vec{g}\dotp\vec{r}}{\vec{r}\dotp\vec{r}}\right)\vec r\\
