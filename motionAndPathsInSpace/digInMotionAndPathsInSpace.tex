...
$$\vec{p}(t) = \vector{(350\cos \theta)t, -16t^2+(350\sin\theta)t+3}.$$ We need to find $\theta$ so that $\vec{p}(t) =\vector{25,6}$ for some value of $t$. That is, we want to find $\theta$ and $t$ such that 
...
{Let $\vec{v}(t)$ be a velocity function for a moving object. The distance traveled by the object on $[a,b]$ is:
...
%Given a position function $\vec{p}(t) = \vector{f(t),g(t)}$, Theorem \ref{thm:distance_traveled}
...
A particle moves in space with position function $\vec{p}(t) = \vector{t,t^2,\sin (\pi t)}$ on $[-2,2]$, where $t$ is measured in seconds and distances are in meters. Find:
\begin{enumerate}
...
{\begin{enumerate}
	...
	$$\text{average speed} = \frac{1}{2-(-2)}\int_{-2}^2 \norm{\vec{v}(t)}\d t \approx \frac14 12.88 = 3.22\text{m/s}.$$
...
that is, it is the average value of $\vec{p}'(t)$, or $\vec{v}(t)$, on $[a,b]$.\\
