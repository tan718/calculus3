...
\[
z = F^{(2,0)}(\vec{c})(x-c_1)^2 + F^{(0,2)}(\vec{c})(y-c_2)^2 + F^{(1,1)}(\vec{c}) (x-c_1)(y-c_2) + F(\vec{c})
\]
...
z = F^{(2,0)}(\vec{c})(x-c_1)^2 + F^{(0,2)}(\vec{c})(y-c_2)^2 + F^{(1,1)}(\vec{c}) (x-c_1)(y-c_2) + F(\vec{c})
\]
Without any loss of generality, we may assume that $\vec{c}
=\vec{0}$. All this assumption does is ``move'' the quadric surface to
the origin.

\subsection{The mixed derivative is zero}
