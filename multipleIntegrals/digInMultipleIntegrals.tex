...
\begin{example}
  ...
  \begin{align*}
    ...
    &= \eval{\frac74y^2+e^y}_{\answer[given]{1}}^{\answer[given]{2}}\\
    &=\answer[given]{\frac{21}{4}+e^2-e}.
  \end{align*}
  \end{explanation}
\end{example}

In our next example, we will see that it is sometimes easier to
integrate with respect to one variable or the other.
    ...
    \begin{tikzpicture}
      \begin{axis}%
        [
          tick label style={font=\scriptsize},%axis on top,
	  axis lines=center,
	  view={70}{20}, %% rotate around / up and down
	  name=myplot,
	  ymin=-.1,ymax=1.1,
	  xmin=-.1,xmax=1.1,
	  zmin=-.1, zmax=3,
	  ...
	  zlabel={\scriptsize $z$},
        ...
        \addplot3[domain=0:1,%fill=white,
          penColor2,very thick,samples=20,samples y=0] ({1},{x},{exp(x)});
        
        %% \addplot3[domain=0:1,%fill=white,
        %%   penColor3,very thick,samples=20,samples y=0] ({1},{x},{x*exp(x)});
        
        %% \addplot3[domain=0:1,%fill=white,
        %%   penColor4,very thick,samples=20,samples y=0] ({x},{1},{1*exp(x)});
        
        \addplot3[domain=0:1,%fill=white,
          penColor5,very thick,samples=20,samples y=0] ({x},{1},{x*exp(x)});
