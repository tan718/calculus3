...
only considering simple regions, by this we mean rectangles.
...
If we want to compute the (signed) volume of this surface, over a
rectangular region, say the rectangle defined by
\[
R = \{(x,y):\text{$a\le x\le b$ and $c\le y\le d$}\}
\]
we break $R$ in to arbitraryly small rectangles, say $m$ rectangles in
the $x$-direction, and $n$ rectangles in the $y$-direction, and make a
box of height $F(x_i*,y_j*)$ where $(x_i^*,y_j*)$ is a point in
$(i,j)$-rectangle. Finally we sum the volumes of these boxes together:
\begin{image}
  \begin{tikzpicture}
    \begin{axis}%
      [width=175pt,height=200pt,
...
$y$-direction go to infinity, we find the exact volume enclosed by our
surface when bounded by the region $R$.
...
\[
\int_R \d A = \lim_{m\to \infty}\lim_{n\to\infty}\sum_{i=1}^m\sum_{j=1}^n \chi_{R}(x)\Delta A
\]
