...
When the intgrand of a double integral is $1$, like this
\[
\int_R\d A
\]
...
\begin{theorem}[Fubini]\index{Fubini's Theorem}
  ...
  \item If
    ...
    \]
    where $g_1$ and $g_2$ are continuous functions on $[a,b]$, and $H_1$ and $H_2$ are continous on the region
    \[
    ...
    \]
    There are six reordings total with three variables. We will spare
    the young mathematican the details, and trust that you will sort
    it out.
\end{itemize}
\end{theorem}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    \begin{align*}
      \int_{0}^{4} \int_{0}^{-3x+4} \int_{0}^{\frac{12-3x-4y}{6}}\d z \d y\d x &=\int_{0}^{4} \int_{0}^{-3x+4} \frac{12-3x-4y}{6} \d y \d x\\
      &= \int_{0}^{4}  \eval{\frac{12-3x-2y^2}{6}}_{0}^{-3x+4} \d x\\
      &= \int_{0}^{4}  \left(\frac{12-3x-2(-3x+4)^2}{6} - \frac{12-3x}{6}\right) \d x\\
      &= \eval{3x-3x^2/4+x^3/16}_0^4 \d x\\
      &=4.
    \end{align*}    
  \end{explanation}
\end{example}
