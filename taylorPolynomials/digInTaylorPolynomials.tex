...
\begin{align*}
  ...
  &=P_0(\vec{x}) + \eval{\eval{\frac{(\vec{a}\dotp \grad) F(\vec{x})}{1!}}_{\vec{x}=\vec{c}}}_{\vec{a}=\vec{x}-\vec{c}}\\
...
\]
and this is the tangent ``plane'' for $F$ at $\vec{x}= \vec{c}$ Now
let's look at the $2$nd degree Taylor polynomial:
\[
P_2(\vec{x}) = F(\vec{c})
