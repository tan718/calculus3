...
\begin{image}
  \begin{tikzpicture}[scale=2,every node/.style={transform shape}]
    \node at (0,0) {
      $\color{green!70!black!70!blue}\int_{\color{red!50!black}R} \color{purple!50!blue!90!black}F\color{blue!70!green} \d A$
    };
  \end{tikzpicture}
\end{image}
...
\begin{question}
  Consider the polar function described below:
  \begin{image}
    \begin{tikzpicture}
  ...
  \begin{hint}
    Remember,
    \[
    \int_R F(r,\theta)\d A = \int_{\pi/4}^{5\pi/4}\int_{0}^1 F(r,\theta) r\d r\d \theta.
    \]
    Moreover, let $r$ be determined by the ``outer-value'' of $r$ for
    the box containing the value for $F$.
  \end{hint}
\end{question}
